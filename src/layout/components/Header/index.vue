...
<script setup lang="ts">
  ...
  import { computed, reactive, unref, ref, watch } from "vue";
  ...
  const nickname = ref<string>(store.$state.currentUser.nickname);

  watch(store.$state, (state) => {
    if (state.currentUser.nickname !== nickname.value) {
      nickname.value = state.currentUser.nickname;
      console.log("!!watch");
    }
  });
